            ...
            <div class="card-footer">
              <div class="col-12 col-md-2 offset-10">
                <div class="form-group mt-4">
                  <input type="submit" class="btn btn-sm btn-info add_material px-4" value="Add">
                  <a type="button" class="btn btn-sm btn-danger cancel px-4" id="cancel" href="<?= site_url(); ?>admin/material/index">Cancel</a>
                </div>
              </div>
            </div>
            </form>
...
<script type="text/javascript">
  $(document).ready(function() {
    $('#uploadForm').submit(function(e) {
      e.preventDefault();
      if($('#material_name').val() && $('#imageToUpload').val()){
        var formData = new FormData(this);
        $.ajax({
          url: '<?= site_url(); ?>admin/material/save_material',
          type: 'POST',
          data: formData,       
          cache: false,
          contentType: false,
